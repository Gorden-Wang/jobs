...
mongo.prototype = {
    ...
    insert: function (data,nextcallback) {
        ...
        that.openDB(data, 'hr', function (err, collection) {
            ...
            function check(i,data, coll, callback) {
                ...
                coll.findOne({code:data.code}, function (err, docs) {
                    if (err) {
                        util.console.log("findOne error :" + err)
                        return;
                    }
                    ...
                    util.console.log("next call back start")
            ...
            function insert(i,data, coll) {
                ...
                coll.insert(data, function (err, docs) {
                    ...
                    util.console.log("insert success");
    ...
    find: function (data) {
        var that = this;
        util.console.log("next call back in")
        that.openDB(data, 'hr', function (err, collection) {
    ...
    _update:function(id,data){
        var that = this;
        that.aliCollection.update({"id": id}, {$set: data},function(err,docs){
            if(err){
                util.console.log("update err:"+JSON.stringify(err));
            }

            util.console.log(docs)

            util.console.log("update success");
        });
    }
